...
- Ordinary HTTP uses only the legacy port from Host/`E2b-Sandbox-Id + E2b-Sandbox-Port`. It does not parse `E2b-Sandbox-Service`; that header remains an application header in the inner request.
...
After the router's local authentication gate, a route with `NodeSandboxID + DataEndpoint` does not wait for READY in the registry: paused/starting opens a one-use node CONNECT immediately, just like ready. The final node proxy follows `LookupRoute → authorize → BeginParking → ActivateRoute/Wake → fresh Route → dial`. Authorized parking is therefore counted only at the final node, without duplicate router/registry accounting. In ordinary `enforce` mode, invalid tokens fail before node CONNECT/Reserve. File-signature validation and explicit token checks on `/files` remain enforced in every mode. Exec is always enforced. For ordinary requests, `log` records a local mismatch and continues, while `off` skips that local token check; these modes do not disable the final node's authentication or allow an unauthorized request to Wake/Resume/park/dial the backend.
