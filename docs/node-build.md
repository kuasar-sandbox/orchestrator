...
For the primary multi-pool NUMA deployment use case, see [Node §5.3](node.md#numa-deployment) for the two-node runner/Builder configuration, systemd placement drop-ins, installation order and effective-placement checks. Select one Builder pool only after global execution admission; all A/B/C phases that actually execute stay under that Builder unit and its host placement. Builder size is a prewarm target, not a per-NUMA concurrency/memory budget. Later Sandboxes created from the output select their own runner; an artifact does not pin them to the Build host's NUMA node.
