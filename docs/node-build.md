...
|---|---|---|
| Register | POST /v3/templates → 202 | Body accepts name/tags/profile?/cpuCount/memoryMB/metadata?/envVars?/secure?. CPU/memory define immutable Build.Resources for admission and A/B sandbox specifications; final target Sandbox resources are resolved independently. X-Kuasar-Sandbox-Builder.resources may assert identical CPU/memory and add storage; disagreement returns 400. Builder target is register-only immutable input. X-Kuasar-Sandbox-Resource configures final template resources, not A/B. Profile defaults e2b and accepts e2b/bare. Response exposes requested target, auto when omitted |
...
| `builder.diff_template` | — | Sparse ext4 formatted with `mkfs.ext4 -O ^has_journal` for pull cache, step changes and export scratch; suggested size at least three times the largest expected image |
...
Build.Resources remains immutable and supplies both admission/accounting and resolveBuildExecutionResources for A/B Sandboxes. resolveBuildTargetResources independently resolves the final Sandbox. sandbox-ctl owns actual VMM memory.max, memory.high, CPU capacity ceiling and weight; ctl remains isolated in its sibling cgroup. Recovery checks execution identity, liveness, durable preparation, accepted result, deadline, phase and cleanup ownership, without reading service/slice resource properties.
...
1. Arrange a maintenance window, pause new Build submissions, and let existing Builds finish through their normal lifecycle. Verify exact Build status, execution claims and runtime/phase ownership; do not terminate live Builds or release unfinished claims for this cleanup. Keep a consistent database/config backup before upgrading writers. If upgrading with live Builds, leave their old runtime settings alone until they finish; recovery no longer rejects them based on those values.
2. With `units.install=true`, the new conductor updates its project-generated slice file. With `units.install=false`, operators update their own files. Inspect `FragmentPath`, `DropInPaths` and file contents first; remove only old project-generated CPUQuota/MemoryMax directives whose source is confirmed. Preserve operator drop-ins and unrelated unit properties.
3. Inspect each completed old run by exact unit name. Old `SetResources` used runtime-only D-Bus properties, normally backed by the two files below. Verify their provenance and that the Build/unit has no remaining execution or cleanup owner before removing either. A missing or differently named file requires inspection, not a wildcard or `systemctl revert`. Do not apply these commands to a template, all instances or an ancestor slice.
...
4. Inspect actual cgroup values for the updated slice and a fresh execution, using `systemctl show --value -p ControlGroup <exact-unit>` to locate them. In an isolated deployment without operator policy, parent service/slice `cpu.max` starts with `max`, `memory.max=max`, and ctl remains outside VMM limits. Keep VMM `memory.max`, `memory.high`, capacity ceiling and weight under sandbox-ctl's existing policy. A completed service may already have no cgroup; its absence is not evidence about a later instance. Resolve any retained setting by its confirmed source, without overwriting operator policy or changing arbitrary ancestor slices.

`Store.Open` independently drops only `builds.enforcement_status` when present in a supported schema. It preserves business rows, credentials, accepted results, claims and ownership, and fails startup on migration error. Repeated Open is safe. The pre-#300 rejection boundary remains. The new schema/API/BuildView contain no systemd enforcement field; `storageEnforcement: admission-only` retains its separate meaning. Old writers still reference the removed column and are not a supported mixed-version or direct rollback path; coordinate writer versions and any rollback with the consistent pre-upgrade backup.
...
The shared child-start boundary only injects the trusted VMM/readiness descriptors, derives their child FD numbers and starts the command. It closes the parent readiness copy on either success or failure, borrows the VMM descriptor, and leaves exactly one Wait to the caller. Command construction, environment, stage cancellation, readiness consumption and phase teardown remain Build responsibilities; sharing the spawn boundary does not change the Build lifecycle.
...
**Two-level admission:** in one SQLite transaction, Register checks count/CPU/memory/storage against builder.admission.registration and inserts/charges the immutable definition. Trigger only moves registered→waiting. Scheduler uses stable `waiting_sequence` FIFO (durable successful Trigger commit order) and transactionally claims builder.admission.execution. Insufficient capacity stays waiting until queue_ttl makes it durably terminal. After claiming, durably bind the exact builder run ID, then publish assignment. Execution CPU/memory remain aggregate admission limits; storage is admission-only in V1. Builder status/metrics expose configured/durable usage, headroom, queues and rejection/expiry counters. Legacy max_concurrent/cpu_quota/memory_max/vcpu/memory settings are rejected. Both atomic admission writes also reject aggregate int64 overflow in unlimited dimensions; this preserves ledger arithmetic without adding a configured resource limit.
...
Build rows store no directory paths. Derive both Build directories from BuildID and current RunRoot/BaseRoot. Every terminal path fences the exact unit/cgroup, waits for any dynamic phase reservation, detaches and removes both directories. Durable ownership remains until cleanup is complete; terminal commit atomically clears RunID/result and releases the execution claim. Phase child cleanup is not final correctness authority. Node database, config socket and runner pidfile are outside object directories and unaffected by object RemoveAll.
...
The existing process-local execution owner is installed before claim publication. Intent stops further trigger/claim/assignment/bootstrap/prepare/phase/result advancement, and notifies that owner to stop the exact RunID unit, including phase VMs and helpers. Host preparation exits or rolls back its uncommitted resources before that same owner cleans the exact network, runtime entry, BuildRunDir and BuildBaseDir. After unit stop, cleanup reads the durable PhaseSandboxID and waits for the existing resource controller reservation to disappear. If runtime fields are the only remaining ownership, they stay durable through both directory removals and are conditionally released in PutBuildTerminal. Build locks cover short identity and database/event commits, never unit stop or directory removal. Cleanup CAS remains valid after cancellation. HTTP disconnect does not revoke accepted intent; cleanup uses independent bounded external-operation contexts.
...
Cleanup completes before PutBuildTerminal releases the claim; capacity notifications follow the commit immediately, and conditional hard deletion follows under the existing registration/retention fence as a separate commit. A registration replay waiting on terminal publication cannot reinsert the deleted identity. A final DELETE failure preserves delete intent without charging execution again. Immediate retry, bounded lifecycle compensation and startup recovery continue without waiting for terminal_ttl. Stop/unit-liveness readback/detach/directory/terminal failures retain necessary ownership for retry. Startup handles cancel/delete before adopting a pipeline: stop live units, clean exited units, delete owner-free marked terminal rows, never reread source artifacts for cancelled work or reset the original execution deadline.
