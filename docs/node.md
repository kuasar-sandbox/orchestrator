...
远程内存 Prefetch **没有节点统一配置**。`sandbox.restore.file_refs` 只管理节点本地文件引用
的信任边界;是否 Prefetch 由每个 sandbox 的 `kuasar-sandbox.restore` 请求命名空间决定
(§4.6),二者不能互相覆盖。
...
| 操作 | 方法 + 路径 | 要点 |
|---|---|---|
| create | `POST /sandboxes` → 201 | body `{templateID, timeout, metadata, envVars}` + 可选 `X-Kuasar-Sandbox-*` 头(timeout 缺省取 `sandbox.timeout_sec`,默认 300s);回 `{sandboxID, templateID, clientID, domain, envdVersion, envdAccessToken, trafficAccessToken, alias}` |
...
| `restore` | host restore policy 的 `prefetch`(空/`off`/`memory`);仅 restore 启动渲染,不含节点托管的 `file_refs` |

`restore` 命名空间只接受严格 JSON object。单实例显式启用的两种等价请求形态:
...
未知字段、非 JSON、非字符串 `prefetch` 和 `file_refs` 均在创建副作用前回 **400**。
未提供时默认关闭;显式 `{"prefetch":"off"}` 可覆盖模板的 `memory`。
orchestrator 只传递每个 sandbox 的显式策略,不判断本地/远程、单层/多层或底层能力。
显式 `memory` 对单层和多层 restore 均会渲染;最终执行资格由 sandboxer 判断。多层顶层更接近
活动工作集,是推荐场景,不是强制门槛。
...
- **network 随快照**:渲染时把已解析逻辑网络注入
  ...
  token 同样携带 metadata。
- **restore policy 不随快照**:`kuasar-sandbox.restore` 留在 sandbox/template metadata 中。
  image cold boot 不把它渲染进运行 YAML;pause 后 resume、snp create 和 migration import
  根据各自持久化 metadata 重新渲染。sandboxer 不把该 host policy 写进 snapshot.cfg。
...
- **每实例配置**(create/构建经 metadata + `X-Kuasar-Sandbox-*` 头,命名空间化,详见
  ...
  `restore.prefetch` 则只随 orchestrator metadata,不会被快照继承;冷启省略,后续 restore
  才按该 sandbox 的显式策略渲染。
...
| `e2e_execute.sh` | 从已建模板冷启真实 microVM、guest 内 exec、pause(remote snapshot)→ resume 全链路;create 经 `X-Kuasar-Sandbox-Network` 注入 hostname,并经 `X-Kuasar-Sandbox-Restore` 持久化 memory Prefetch;断言 cold YAML 省略、resume YAML 渲染且 sandboxer 实际启动 Prefetch(§4.6) | `test-e2e-execute` |
