# proxy — orchestrator 数据面转发层

## 1. 概述

数据面 proxy 是沙箱流量的 **L7 反代**:把外部(e2b SDK/CLI 或端口转发)请求按
`(sid, port)` 路由到目标沙箱的 guest envd 或 floatingip 用户端口。控制面(e2b API、
生命周期、密钥、模板构建)由 `orchestrator-ctl serve` 承载,见
[orchestrator.md](orchestrator.md);本文档只讲数据面**转发层**:路由判定、部署形态、
...
- **转发层与控制面分离**:proxy 只做"路由判定 + 鉴权 + 转发",不持久化、不调度;
  路由权威在 orchestrator,proxy 持只读缓存。
- **拓扑反转,worker 主动注册**:外置 worker 拨 orchestrator 注册并拉路由,orchestrator
  从不主动拨 worker——数据面字节流不经 orchestrator。
...
- 依赖 orchestrator 的 **config-socket plugin 平面**(注册 + routesync,见
  orchestrator.md §6)。
- 依赖 **sandbox-vswitch** 的 mgmt-extract 把 MMDS VIP 直译到本进程(§8)。
- 上游是 guest **envd**(经 host-UDS)或沙箱 **floatingip**;envd 协议见 orchestrator.md §4.3。
- 须与 orchestrator 同节点(本地拨 envd-UDS / floatingip)。

## 2. 命令行:`orchestrator-ctl proxy`

外置数据面 worker(部署模式见 §5)。运维带外起(`deploy/orchestrator-proxy@.service`),
与 orchestrator 同节点。
...
| 参数 | 默认 | 说明 |
|---|---|---|
| `--config-socket` | (必填) | orchestrator 的 config-socket UDS:worker 在其 plugin 平面注册并同步路由(orchestrator.md §6、本文 §6) |
| `--id` | (必填) | 本 worker 的 plugin id,每 worker 唯一(同 id 二次注册顶掉前者) |
| `--socket` | (必填) | 本 worker 服务兜底网关转发的 UDS(注册时上报给 orchestrator) |
| `--data-listen` | 空 | 数据面入口,**SO_REUSEPORT**(多 worker 共享同一端口);空 = 仅 UDS 服务 |
| `--tls-cert/--tls-key` | 空 | 数据面 TLS(与 orchestrator 同一张通配证书);空 = h2c |
| `--auth` | `enforce` | 数据面鉴权回退值,仅在 orchestrator 策略到达前生效(§7) |
...
数据面相关字段(完整配置表见 orchestrator.md §3):
...
`(sid, port)` → 路由判定 → 校验 `X-Access-Token`(§7)→ 转发。两种部署形态共用同一
路由判定函数:
...
提供;serve 如何按模式装配数据面见 orchestrator.md §9):
...
- **注册即订阅**:worker 经 `--config-socket` / `--id` 在 plugin 平面(orchestrator.md §6)
  注册 `subscribe=route_wake` + `proxy{socket}`,持挂连接 = 租约 + 路由流。orchestrator
  不再拨 worker——拓扑反转后它只是连接的应答方与路由权威。断连即反注册;同 id 二次注册
  顶掉(断链)前者。同一平面也接受**非 proxy 观察者**(如平台 agent,`subscribe=route`),
  借此感知沙箱状态(含暂停态本地 / 远程,orchestrator.md §8.1)而不参与数据面转发。
...
- worker 由运维带外管理(`deploy/orchestrator-proxy@.service`),serve 不自动安装;须与
...
- 传输:订阅者拨 orchestrator 的 config-socket,发 `PUT /internal/plugin/{id}/register`
  (plugin 平面,orchestrator.md §6,`plugin_pidfile` / socket 0600 鉴权);单 HTTP/2
...
- `RouteEntry = {sid, profile, template_id, state(running|paused|dead), envd_uds,
  ...
  - `snap_loc`:running/dead 为空,否则 `local`(节点绑定的本机快照)或 `remote`
    (已上传、可移植)。观察者据此判定迁移(orchestrator.md §8.1);迁移 token 仍由
    export-sandbox 按需铸造,不随路由广播。
  - `mmds_secret`:该沙箱的 MMDS 会话签名密钥(hex),由 `manifest_key + sid` 确定性派生
    (§8),不服务 MMDS 的订阅者忽略即可。
- 容错:连接断 → 订阅者指数退避重连重注册(0.2s 起、5s 封顶),重连即重新 `register`
  + 重新同步(逐条 upsert + bookmark);订阅积压 → orchestrator 掐掉该订阅、订阅者重连
  重同步。Wake 的 resume 由 orchestrator 端单飞去重。
...
- 数据面 token = `envdAccessToken`,头 **`X-Access-Token`**(与原版 e2b 一致;secure
  ...
  orchestrator.md §7)→ 回 SDK → 随路由分发;**proxy 逐请求校验**其与该沙箱 token
  一致(常数时间比较)。
...
- **worker 崩溃**:持挂连接断 → orchestrator 反注册该 worker;SO_REUSEPORT 下其余
...
- **resume 单飞**:park / Wake 触发的 resume 由 orchestrator 端单飞去重(internal 直接、
  external 经 routesync `Wake` 上行),并发请求只触发一次恢复(orchestrator.md §8)。
...
- [orchestrator.md](orchestrator.md) — 控制面:e2b API、生命周期与状态机、密钥模型
  (`envdAccessToken`/`MmdsSecret` 派生,§7)、模板构建,以及 serve 如何按 `proxy.mode`
  装配数据面(§9)。
- [node.md](node.md) — 节点资源控制协议(与数据面正交)。
