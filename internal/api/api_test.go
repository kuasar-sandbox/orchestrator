...
func TestMergeConfigHeaders(t *testing.T) {
	...
	if got := mergeCreateConfigHeaders(nil, h); got[sandboxcfg.NsMMDS] != `{"version":1,"routes":[{"path":"/x","type":"static","data":"d"}]}` {
	...
	got = mergeCreateConfigHeaders(nil, emptyMMDS)
	...
	got = mergeCreateConfigHeaders(metadataMMDS, header("X-Kuasar-Sandbox-MMDS", `{"version":1,"routes":[{"path":"/from-header","type":"static","data":"h"}]}`))
