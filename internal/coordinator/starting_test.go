...
func buildStartingRecord(t *testing.T, buildID string, candidates []cluster.PlacementCandidate, rejected []uint32) cluster.BuildRecord {
	...
	spec, err := cluster.MarshalBuildDispatchSpec(cluster.BuildDispatchSpecV1{
		...
		Request: cluster.NodeRequestEnvelopeV1{Version: cluster.NodeRequestEnvelopeVersionV1, Method: "POST", Path: "/v3/templates", Body: []byte(`{"cpuCount":1,"memoryMB":512,"metadata":null,"name":"","profile":"bare","tags":null}`)},
	})
	if err != nil {
		t.Fatal(err)
	}
