...
func TestWorkerResolveWakesAndWaitsForSharedUpdate(t *testing.T) {
	...
	worker := NewWorkerView(tbl, updates, master.Wake, 500*time.Millisecond, nil, 0, nil)
	master.BeginSync()
	master.Bookmark()
...
func TestWorkerRouteSelectsPurposeSpecificAccessToken(t *testing.T) {
	...
	tbl.Bookmark()
	view := NewWorkerView(tbl, nil, nil, time.Second, nil, 0, nil)
...
func TestWorkerKnownUnsupportedServiceDoesNotWakePausedRoute(t *testing.T) {
	...
	tbl.Bookmark()
	worker := NewWorkerView(tbl, nil, master.Wake, 50*time.Millisecond, nil, 0, nil)
