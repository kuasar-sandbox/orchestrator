...
// ValidateRegistryLayoutTransition checks the cross-artifact constraints that cannot
// be proven from one registryLayout in isolation. A retained member and retained
// shard replica keep their exact transport identity; replacing either side
// requires a new member and a never-before-used replica ID for that shard.
func ValidateRegistryLayoutTransition(previous, next RegistryLayout) error {
	...
	for _, member := range next.Members {
		...
		if owner := previousInternalEndpoints[member.InternalEndpoint]; owner != "" ||
			previousRaftEndpoints[member.RaftEndpoint] != "" {
