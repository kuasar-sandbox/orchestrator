...
func TestRegistryLayoutTransitionKeepsRetainedTransportAndReplicaIdentity(t *testing.T) {
	...
	next.Members = append([]RegistryMember(nil), previous.Members...)
	...
	for name, mutate := range map[string]func(*RegistryLayout){
		"retained endpoint": func(registryLayout *RegistryLayout) {
			registryLayout.Members = append([]RegistryMember(nil), registryLayout.Members...)
			registryLayout.Members[0].InternalEndpoint = "https://registry-a-next:9443"
		},
		"reused endpoint": func(registryLayout *RegistryLayout) {
			registryLayout.Members = append([]RegistryMember(nil), registryLayout.Members...)
			registryLayout.Members[3].RaftEndpoint = previous.Members[2].RaftEndpoint
			registryLayout.Members[2].RaftEndpoint = "registry-c-retired:63001"
		},
		"retained replica changed": func(registryLayout *RegistryLayout) {
			...
			registryLayout.DataShards[0].Replicas[0].ReplicaID = 5
		},
		"predecessor replica reused": func(registryLayout *RegistryLayout) {
			registryLayout.DataShards = append([]ShardPlacement(nil), registryLayout.DataShards...)
			registryLayout.DataShards[0].Replicas = append([]ReplicaPlacement(nil), registryLayout.DataShards[0].Replicas...)
			registryLayout.DataShards[0].Replicas[2].ReplicaID = 3
		},
	} {
		t.Run(name, func(t *testing.T) {
