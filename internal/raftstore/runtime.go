...
type RemoteSystemClient interface {
	...
	ConfirmRecoveryPermitDrain(context.Context) (SystemState, error)
...
func openRuntime(
	...
	runtime := &Runtime{
		...
		outboxAckVerifier: options.OutboxAckVerifier, terminalVerifier: options.TerminalVerifier,
...
func (r *Runtime) RefreshPermit(ctx context.Context) (PermitGrant, error) {
	if !r.HasLocalSystemReplica() {
		...
		if state.LastApplied < grant.CommitIndex || state.Identity() != grant.PermitIdentity ||
			...
			grant.ServeGate != state.ServeGate || grant.WriteGate != state.WriteGate ||
			grant.CutoverGate != state.CutoverGate || grant.RecoveryClosed != (state.Recovery == nil) {
