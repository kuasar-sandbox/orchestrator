...
func (g *switchGate) AllowSessionWork(ServeIdentity, PermitOperation) bool {
	g.mu.RLock()
	allowed := g.allowed
	g.mu.RUnlock()
	select {
...
func (*switchGate) AuthorizeNodeSession(ServeIdentity, Registration) error { return nil }
...
type blockingCommandEndpoint struct {
...
func (e *blockingCommandEndpoint) FenceStaleSession() {
	e.fenceOnce.Do(func() { close(e.fenced) })
}

func (*blockingCommandEndpoint) AdmitAndDispatch(context.Context, DispatchCommand) (DispatchReply, error) {
	return DispatchReply{Outcome: cluster.DispatchAcceptedAdmitted}, nil
}

func (e *blockingCommandEndpoint) SendNodeCommand(_ context.Context, command *routesync.Command) (routesync.CmdAck, bool, error) {
	close(e.commandStarted)
	<-e.releaseCommand
	return routesync.CmdAck{CmdID: command.CmdID, Status: routesync.AckAccepted}, true, nil
}

func TestHolderSerializesNodeCommandWithSessionReplacement(t *testing.T) {
	registration := testRegistration("node-1", 7, 10, "10.0.0.1:8443")
	holder, err := NewHolder("registry-a", 1, nil, testGate(true), nil, newTestEnrollmentAuthority(registration))
	if err != nil {
		t.Fatal(err)
	}
	endpoint := &blockingCommandEndpoint{
		commandStarted: make(chan struct{}), releaseCommand: make(chan struct{}), fenced: make(chan struct{}),
	}
	if _, err := holder.Register(context.Background(), registration, endpoint); err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		_, _, err := holder.SendNodeCommand(
			context.Background(), testServeIdentity(), registration.NodeID, registration.NodeEpoch,
			registration.DataEndpoint, &routesync.Command{CmdID: "delete-1", Kind: routesync.CmdSandboxDelete},
		)
		commandDone <- err
	}()
	<-endpoint.commandStarted
	...
	select {
	case err := <-registerDone:
		t.Fatalf("session replacement completed during command: %v", err)
	case <-endpoint.fenced:
		t.Fatal("old session was fenced during command")
	case <-time.After(20 * time.Millisecond):
	}
	close(endpoint.releaseCommand)
	if err := <-commandDone; err != nil {
		t.Fatalf("node command: %v", err)
	}
	if err := <-registerDone; err != nil {
		t.Fatalf("replacement: %v", err)
	}
...
func TestKeyLeaseRevisionIsScopedToExactFingerprintStream(t *testing.T) {
	registration := testRegistration("node-1", 7, 10, "10.0.0.1:8443")
	holder, err := NewHolder("registry-a", 1, nil, testGate(true), nil, newTestEnrollmentAuthority(registration))
	if err != nil {
		t.Fatal(err)
	...
	rotated := first
	rotated.AuthKey = testKeyMaterial(strings.Repeat("d", 64))
	if _, sent, err := holder.InstallKeyLease(
		context.Background(), testServeIdentity(), registration.NodeID, registration.NodeEpoch, registration.DataEndpoint, rotated,
	); err != nil || !sent {
		t.Fatalf("independent rotated lease sent=%v err=%v", sent, err)
	}
	if len(endpoint.wire) != 2 {
		t.Fatalf("independent fingerprint streams sent %d commands, want 2", len(endpoint.wire))
	}
}

func TestRejectedKeyDropDoesNotAdvanceStreamRevision(t *testing.T) {
	registration := testRegistration("node-1", 7, 10, "10.0.0.1:8443")
	holder, err := NewHolder("registry-a", 1, nil, testGate(true), nil, newTestEnrollmentAuthority(registration))
	if err != nil {
		t.Fatal(err)
