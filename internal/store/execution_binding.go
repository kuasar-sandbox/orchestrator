...
func (s *Store) CASExecutionBinding(
	...
	if workflow != nil {
		...
		if workflow.LatestEvent != nil {
			...
			// An ACK for the source Registry History Generation projection cannot acknowledge the
			// same sequence under the target Binding and Registry History Generation.
			workflow.AckedEventSeq = 0
			notifyEvent = workflow.EventSeq != 0
		}
...
func validateWorkflowBinding(
	...
	if event.ObjectKind != executionKindName(workflow.Kind) || event.ObjectID != workflow.ObjectID ||
		...
		event.RegistryGeneration != registryGeneration || event.Binding != opaque ||
		event.BindingDigest != digest || event.EventSeq != workflow.EventSeq {
