...
var (
	...
	ErrBuildTriggerConflict      = errors.New("store: Build trigger conflicts with the accepted trigger")
)
...
func (s *Store) BuildQueueDepth(ctx context.Context, nodeID string, nodeEpoch uint64) (uint64, error) {
	if nodeID == "" || nodeEpoch == 0 {
		return 0, errors.New("store: node identity is required for Build queue depth")
	}
	tx, err := s.db.BeginTx(ctx, &sql.TxOptions{ReadOnly: true})
	if err != nil {
		return 0, err
	...
	defer tx.Rollback()
	depth, err := buildQueueDepthTx(ctx, tx, nodeID, nodeEpoch)
	if err != nil {
		return 0, err
	}
	if err := tx.Commit(); err != nil {
...
func (s *Store) WorkflowWake() <-chan struct{} { return s.workflowWake }
...
func executionEventFor(
	...
	event := &routesync.ExecutionEvent{
		...
		RegistryGeneration: binding.RegistryGeneration, Binding: record.OpaqueBinding,
