...
func TestDeletedSandboxRemovesBusinessRowButRetainsDurableEvent(t *testing.T) {
	...
	sandbox := workflowSandbox(dispatch.ObjectID)
	if _, err := st.CommitSandboxEvent(ctx, sandbox, nodeexec.EventUpdate{
		State: string(clusterstate.WorkflowRouteReady),
	}); err != nil {
		t.Fatal(err)
	}
	deleted, err := st.CommitSandboxEvent(ctx, sandbox, nodeexec.EventUpdate{State: "DELETED"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := st.CommitSandboxEvent(ctx, sandbox, nodeexec.EventUpdate{State: "DELETED"}); err != nil {
