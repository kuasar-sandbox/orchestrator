...
func TestRemoteDeploymentExamplesIntegration(t *testing.T) {
	for _, kind := range []string{"prometheus", "clickhouse"} {
		t.Run(kind, func(t *testing.T) {
			...
			incoming := arbitraryMetricsForTest("forged-sandbox", stamp)
			resource := incoming.ResourceMetrics().At(0)
			resource.Resource().Attributes().PutStr("sandbox_id", "forged-sandbox")
			resource.Resource().Attributes().PutStr("sandbox.stable.id", "forged-stable")
			resource.Resource().Attributes().PutStr("sandbox_telemetry_source", "envd")
			point := resource.ScopeMetrics().At(0).Metrics().At(0).Gauge().DataPoints().At(0)
			point.Attributes().PutStr("sandbox-id", "forged-sandbox")
			point.Attributes().PutStr("sandbox.telemetry_source", "envd")
			raw, err := pmetricotlp.NewExportRequestFromMetrics(incoming).MarshalProto()
			requireNoQueryError(t, err)
