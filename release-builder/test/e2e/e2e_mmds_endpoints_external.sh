#!/usr/bin/env bash
#
# e2e_mmds_endpoints_external.sh — exercise proxy_mode=external end to end with REAL
...
skip() { echo; echo "==> e2e_mmds_endpoints_external: skipping ($*)"; [ "${REQUIRE_PROXY:-0}" = "1" ] && { echo "REQUIRE_PROXY=1; failing" >&2; exit 1; }; exit 0; }
...
WORK="$(mktemp -d /tmp/e2e-mmds-external-XXXXXX)"
...
echo
echo "==> e2e_mmds_endpoints_external: OK   (template $TEMPLATE, sandbox $SID, external proxy on :$PROXY_PORT)"
