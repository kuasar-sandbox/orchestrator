#!/usr/bin/env bash
#
# e2e_mmds_endpoints_internal.sh — Phase 2: boot a REAL microVM sandbox from a built template and
...
source "$(dirname "$0")/lib/mmds_endpoints.sh"
...
skip() { echo; echo "==> e2e_mmds_endpoints_internal: skipping ($*)"; [ "${REQUIRE_EXEC:-0}" = "1" ] && { echo "REQUIRE_EXEC=1; failing" >&2; exit 1; }; exit 0; }
...
WORK="$(mktemp -d /tmp/e2e-mmds-internal-XXXXXX)"
...
# ---- relay upstream + store + zot + creds + orchestrator ------------------
mmds_relay_setup
...
echo
echo "==> e2e_mmds_endpoints_internal: OK   (template $TEMPLATE, sandbox $SID)"
