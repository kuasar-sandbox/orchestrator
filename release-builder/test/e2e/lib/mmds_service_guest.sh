#!/usr/bin/env bash
#
# Shared MMDS service-route E2E helpers.
#
# A topology-specific suite starts a real HTTP service on a Unix socket,
# registers operator target "e2e_backend", and declares guest alias
# "e2e_service" at /e2e/service. The lifecycle verifies the real guest path,
# identity-header injection, response passthrough, timeout and size limits,
# backend unavailability/recovery, and sandbox lifecycle transitions.
# This file is sourced and intentionally need not be executable.

MMDS_SERVICE_PATH="${MMDS_SERVICE_PATH:-/e2e/service}"
MMDS_SERVICE_ALIAS="${MMDS_SERVICE_ALIAS:-e2e_service}"
MMDS_SERVICE_TARGET="${MMDS_SERVICE_TARGET:-e2e_backend}"

start_mmds_service_backend() {
    local work="$1"
    MMDS_SERVICE_SOCKET="$work/mmds-service.sock"
    MMDS_SERVICE_STATE="$work/mmds-service.state"
    MMDS_SERVICE_REQUESTS="$work/mmds-service.requests"
    export MMDS_SERVICE_SOCKET MMDS_SERVICE_STATE MMDS_SERVICE_REQUESTS
    printf '%s\n' ok >"$MMDS_SERVICE_STATE"
    : >"$MMDS_SERVICE_REQUESTS"

    python3 - "$MMDS_SERVICE_SOCKET" "$MMDS_SERVICE_STATE" "$MMDS_SERVICE_REQUESTS" <<'PY' &
import http.server, json, os, socketserver, sys, time
socket_path, state_path, requests_path = sys.argv[1:]
try:
    os.unlink(socket_path)
except FileNotFoundError:
    pass

class Handler(http.server.BaseHTTPRequestHandler):
    protocol_version = "HTTP/1.1"
    def log_message(self, *_):
        pass
    def do_GET(self):
        headers = {key.lower(): value for key, value in self.headers.items()}
        with open(requests_path, "a", encoding="utf-8") as log:
            log.write(json.dumps({"method": self.command, "path": self.path,
                                  "headers": headers}, sort_keys=True) + "\n")
            log.flush()
        with open(state_path, encoding="utf-8") as state_file:
            state = state_file.read().strip()
        if state == "slow":
            time.sleep(1)
            status, content_type, body = 200, "text/plain", b"too late"
        elif state == "oversized":
            status, content_type, body = 200, "text/plain", b"x" * 1024
        elif state == "retry":
            status, content_type, body = 429, "text/plain", b"MMDS_SERVICE_RETRY"
        else:
            status, content_type = 200, "application/json"
            body = json.dumps({
                "marker": "MMDS_SERVICE_GUEST_E2E",
                "method": self.command,
                "path": self.path,
                "sandbox_id": headers.get("x-kuasar-sandbox-id"),
                "sandbox_subject": headers.get("x-kuasar-sandbox-subject"),
                "service": headers.get("x-kuasar-mmds-service"),
                "metadata_token_forwarded": "x-metadata-token" in headers,
            }, sort_keys=True).encode()
        try:
            self.send_response(status)
            self.send_header("Content-Type", content_type)
            if state == "retry":
                self.send_header("Retry-After", "7")
            self.send_header("Content-Length", str(len(body)))
            self.end_headers()
            self.wfile.write(body)
        except (BrokenPipeError, ConnectionResetError):
            pass

class Server(socketserver.UnixStreamServer):
    allow_reuse_address = True
with Server(socket_path, Handler) as server:
    server.serve_forever()
PY
    MMDS_SERVICE_BACKEND_PID=$!
    export MMDS_SERVICE_BACKEND_PID
    for _ in $(seq 1 40); do
        [ -S "$MMDS_SERVICE_SOCKET" ] && return 0
        if ! kill -0 "$MMDS_SERVICE_BACKEND_PID" 2>/dev/null; then
            wait "$MMDS_SERVICE_BACKEND_PID" 2>/dev/null || true
            MMDS_SERVICE_BACKEND_PID=""
            return 1
        fi
        sleep 0.05
    done
    stop_mmds_service_backend
    return 1
}

stop_mmds_service_backend() {
    if [ -n "${MMDS_SERVICE_BACKEND_PID:-}" ]; then
        kill -TERM "$MMDS_SERVICE_BACKEND_PID" 2>/dev/null || true
        wait "$MMDS_SERVICE_BACKEND_PID" 2>/dev/null || true
        MMDS_SERVICE_BACKEND_PID=""
    fi
}

restart_mmds_service_backend() {
    local work="$1"
    stop_mmds_service_backend
    start_mmds_service_backend "$work"
}

mmds_service_guest_command() {
    local expected_status="$1" state="$2" marker="$3"
    python3 - "$MMDS_SERVICE_PATH" "$MMDS_SERVICE_ALIAS" "$expected_status" "$state" "$marker" <<'PY'
import base64, json, sys
path, alias, expected_status, state, marker = sys.argv[1:]
program = f'''
import http.client, json
def request(method, target, token=None):
    conn = http.client.HTTPConnection("169.254.169.254", 80, timeout=10)
    headers = {{"X-E2E-Guest-Header": "must-not-be-forwarded"}}
    if method == "PUT":
        headers["X-metadata-token-ttl-seconds"] = "60"
    if token is not None:
        headers["X-metadata-token"] = token
    conn.request(method, target, headers=headers)
    response = conn.getresponse()
    result = response.status, {{k.lower(): v for k, v in response.getheaders()}}, response.read()
    conn.close()
    return result
status, headers, token_body = request("PUT", "/latest/api/token")
assert status == 200, ("token", status, token_body)
status, headers, body = request("GET", {json.dumps(path)}, token_body.decode())
assert status == {int(expected_status)}, ("service status", status, headers, body)
assert headers.get("cache-control") == "no-store", headers
assert headers.get("x-content-type-options") == "nosniff", headers
if {json.dumps(state)} == "ok":
    assert headers.get("content-type") == "application/json", headers
    response = json.loads(body)
    assert response["marker"] == "MMDS_SERVICE_GUEST_E2E", response
    assert response["method"] == "GET" and response["path"] == {json.dumps(path)}, response
    assert response["service"] == {json.dumps(alias)}, response
    assert response["sandbox_id"] and response["sandbox_subject"], response
    assert response["metadata_token_forwarded"] is False, response
elif {json.dumps(state)} == "retry":
    assert body == b"MMDS_SERVICE_RETRY", body
    assert headers.get("content-type") == "text/plain", headers
    assert headers.get("retry-after") == "7", headers
print({json.dumps(marker)})
'''
encoded = base64.b64encode(program.encode()).decode()
print(f'''python3 -c 'import base64; exec(base64.b64decode("{encoded}"))' ''')
PY
}

run_mmds_service_guest_get() {
    local envd_exec="$1" envd_sock="$2" envd_token="$3" work="$4" mode="$5"
    local expected_status="$6" state="$7" marker="$8"
    local command output="$work/mmds-service-guest-$mode-$state.out"
    command="$(mmds_service_guest_command "$expected_status" "$state" "$marker")"
    for _ in $(seq 1 20); do
        python3 "$envd_exec" "$envd_sock" "$envd_token" "$command" >"$output" 2>&1 || true
        if grep -q "$marker" "$output" && grep -q 'EXIT_CODE 0' "$output"; then
            sed 's/^/  mmds-service-guest| /' "$output"
            return 0
        fi
        sleep 0.25
    done
    sed 's/^/  mmds-service-guest| /' "$output" >&2
    return 1
}

mmds_service_assert_request() {
    local requests="$1" subject="$2" local_kind="$3"
    python3 - "$requests" "$subject" "$local_kind" "$MMDS_SERVICE_PATH" "$MMDS_SERVICE_ALIAS" <<'PY'
import json, sys
requests, subject, local_kind, path, alias = sys.argv[1:]
with open(requests, encoding="utf-8") as log:
    rows = [json.loads(line) for line in log if line.strip()]
assert rows, "service backend received no request"
row = rows[-1]
headers = row["headers"]
assert row["method"] == "GET" and row["path"] == path, row
assert headers.get("x-kuasar-sandbox-subject") == subject, headers
local_id = headers.get("x-kuasar-sandbox-id", "")
assert (local_id.startswith(subject + "-g") if local_kind == "generation"
        else local_id == subject), (local_id, subject)
assert headers.get("x-kuasar-mmds-service") == alias, headers
assert "x-metadata-token" not in headers, headers
assert "x-e2e-guest-header" not in headers, headers
PY
}

run_mmds_service_standalone_e2e() {
    local sid="$1" envd_exec="$2" envd_sock="$3" envd_token="$4" work="$5" mode="$6"
    printf '%s\n' ok >"$MMDS_SERVICE_STATE"
    run_mmds_service_guest_get "$envd_exec" "$envd_sock" "$envd_token" "$work" "$mode" \
        200 ok MMDS_SERVICE_OK || return 1
    mmds_service_assert_request "$MMDS_SERVICE_REQUESTS" "$sid" direct || return 1
    printf '%s\n' retry >"$MMDS_SERVICE_STATE"
    run_mmds_service_guest_get "$envd_exec" "$envd_sock" "$envd_token" "$work" "$mode" \
        429 retry MMDS_SERVICE_RETRY_OK || return 1
    printf '%s\n' slow >"$MMDS_SERVICE_STATE"
    run_mmds_service_guest_get "$envd_exec" "$envd_sock" "$envd_token" "$work" "$mode" \
        504 slow MMDS_SERVICE_TIMEOUT_OK || return 1
    printf '%s\n' oversized >"$MMDS_SERVICE_STATE"
    run_mmds_service_guest_get "$envd_exec" "$envd_sock" "$envd_token" "$work" "$mode" \
        502 oversized MMDS_SERVICE_OVERSIZED_OK || return 1

    stop_mmds_service_backend
    run_mmds_service_guest_get "$envd_exec" "$envd_sock" "$envd_token" "$work" "$mode-unavailable" \
        503 unavailable MMDS_SERVICE_UNAVAILABLE_OK || return 1
    restart_mmds_service_backend "$work" || return 1
    run_mmds_service_guest_get "$envd_exec" "$envd_sock" "$envd_token" "$work" "$mode-recovery" \
        200 ok MMDS_SERVICE_RECOVERY_OK || return 1
    mmds_service_assert_request "$MMDS_SERVICE_REQUESTS" "$sid" direct
}
