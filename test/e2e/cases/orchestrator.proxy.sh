...
api: { domain: proxy.e2e.local, listen: "127.0.0.1:$API_PORT" }
...
trap cleanup EXIT
proxy_case_wait_http "$CONDUCTOR" "http://127.0.0.1:$API_PORT/health" api.proxy.e2e.local "$WORK/conductor.log"     || { echo "conductor not ready" >&2; exit 1; }
...
code="$(curl -sS --noproxy '*' -o "$WORK/data.body" -w '%{http_code}' -H 'Host: 49983-unknown.proxy.e2e.local' "http://127.0.0.1:$PORT/health")"
[ "$code" = 401 ] || { cat "$WORK/data.body"; echo "data listener did not enforce WorkerExtension auth: $code" >&2; exit 1; }

code="$(curl -sS --noproxy '*' -o "$WORK/api-data.body" -w '%{http_code}' -H 'Host: 49983-unknown.proxy.e2e.local' "http://127.0.0.1:$API_PORT/health")"
